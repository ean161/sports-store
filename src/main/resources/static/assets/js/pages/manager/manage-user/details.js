async function loadDetails(id) {
    ...
    if (res.data.status === "BANNED") {
        $("#action-btns").html(`<button id="pardon-btn" type="button" class="inline-flex justify-center bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-blue-500  cursor-pointer">Pardon</button>`);
        $("#pardon-btn").attr("onclick", `pardon('${res.data.id}')`);
    } else if (res.data.status === "ACTIVE") {
        $("#action-btns").html(`<button id="ban-btn" type="button" class="inline-flex justify-center bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-red-500  cursor-pointer">Ban</button>`);
