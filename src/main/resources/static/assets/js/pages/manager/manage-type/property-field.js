...
function addPropertyFieldInput(id, htmlId, value = "") {
    let elm = $(`#${htmlId}`);

    elm.append(`<div class="flex space-x-2">
            <input name="field-ids" value="${id}" hidden />
            <div class="flex w-9/10 items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                <input name="fields" placeholder="Title" type="text" class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" value="${value}" />
            </div>
